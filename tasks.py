"""
Invoke tasks for gavel development.
...
#   - .pre-commit-config.yaml `rev: v1.13.1` -> that pins knots, not us
...
def bump_version(c, new_version=None):
    ...
    if changed:
        ...
        for f in sorted(set(changed)):
            print(f"  {f}")
        print("\nNext: review `git diff` and commit.")
    else:
...
def lint(c):
    ...
    c.run("cargo clippy --all-targets --all-features -- -D warnings", pty=True)


@task
def fmt(c):
    """Format the codebase."""
    c.run("cargo fmt --all", pty=True)
